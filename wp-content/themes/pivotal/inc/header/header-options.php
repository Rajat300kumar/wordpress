<?php
    if ( class_exists( 'WooCommerce' ) && is_shop() || class_exists( 'WooCommerce' ) && is_product_tag()  || class_exists( 'WooCommerce' ) && is_product_category()  ) {
        ...
        $header_width_meta = get_post_meta($pivotal_shop_id, 'header_width_custom', true);
        $main_menu_hides   = !empty(get_post_meta($pivotal_shop_id, 'menu-hides', true)) ? get_post_meta($pivotal_shop_id, 'menu-hides', true) : '';
        $main_menu_type    = !empty(get_post_meta($pivotal_shop_id, 'menu-type', true)) ? get_post_meta($pivotal_shop_id, 'menu-type', true) : '';
        $rs_top_search     = get_post_meta($pivotal_shop_id, 'select-search', true);
        $rs_show_cart      = get_post_meta($pivotal_shop_id, 'show-cart-icon', true);
        $rs_offcanvas      = get_post_meta($pivotal_shop_id, 'show-off-canvas', true);
        $rs_phoneicon      = get_post_meta($pivotal_shop_id, 'show-off-phoneicon', true);
        $rs_show_quote     = get_post_meta($pivotal_shop_id, 'show-quote', true); 
        $menu_bg           = get_post_meta($pivotal_shop_id, 'menu-type-bg', true); 

        //Topbar 
        $rs_top_bar = get_post_meta($pivotal_shop_id, 'select-top', true);
...
if ($header_width_meta != ''){
    ...
    $header_width = $pivotal_option['header-grid'];
